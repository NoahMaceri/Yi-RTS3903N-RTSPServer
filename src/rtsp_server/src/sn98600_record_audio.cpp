...
static int snx98600_record_audio_open(struct sonix_audio *audio, const char *filename)
{
	...
	// snd_output_t *output;
	// snd_output_stdio_open(&output, "/var/tmp/sd/test.out", "w");
	// snd_pcm_hw_params_dump(params, output);
	// if (output) {
	// 	snd_output_close(output);
	// }
	int error = 0;
	if ((error = snd_pcm_hw_params_set_access(audio->pcm, params, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
		printf("ERROR: Can't set interleaved mode. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_format(audio->pcm, params, SND_PCM_FORMAT_S16_LE)) < 0) {
		printf("ERROR: Can't set format. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_channels(audio->pcm, params, 1)) < 0) {
		printf("ERROR: Can't set channels. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_rate(audio->pcm, params, SAMPLE_RATE, 0)) < 0) {
		printf("ERROR: Can't set rate. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_period_time(audio->pcm, params, PERIOD_TIME, 0)) < 0) {
		printf("ERROR: Can't set period time. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_period_size(audio->pcm, params, PERIOD_SIZE, 0)) < 0) {
		printf("ERROR: Can't set period size. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_buffer_time(audio->pcm, params, BUFFER_TIME, 0)) < 0) {
		printf("ERROR: Can't set buffer time. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_buffer_size(audio->pcm, params, BUFFER_SIZE)) < 0) {
		printf("ERROR: Can't set buffer size. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params_set_periods(audio->pcm, params, PERIODS_PER_BUFFER, 0)) < 0) {
		printf("ERROR: Can't set periods. %s\n", snd_strerror(error));
	}
	if ((error = snd_pcm_hw_params(audio->pcm, params)) < 0) {
