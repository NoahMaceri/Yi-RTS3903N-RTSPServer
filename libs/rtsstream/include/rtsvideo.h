...
struct rts_isp_vendor_awb_ctrl {
    ...
    struct rts_isp_awb_gain rough_gain;

    const uint32_t current_temperature;

    const struct {
        uint8_t const *r_means;
        uint8_t const *g_means;
        uint8_t const *b_means;
...
int rts_av_query_y_gamma(struct rts_av_gamma **gamma);
int rts_av_get_y_gamma(struct rts_av_gamma *gamma);
int rts_av_set_y_gamma(struct rts_av_gamma *gamma);
int rts_av_query_rgb_gamma(struct rts_av_gamma **gamma);
int rts_av_get_rgb_gamma(struct rts_av_gamma *gamma);
int rts_av_set_rgb_gamma(struct rts_av_gamma *gamma);
void rts_av_release_gamma(struct rts_av_gamma *gamma);
int rts_av_query_ccm(struct rts_av_ccm **ccm);
void rts_av_release_ccm(struct rts_av_ccm *ccm);
int rts_av_get_ccm(struct rts_av_ccm *ccm);
int rts_av_set_ccm(struct rts_av_ccm *ccm);
...
int rts_av_set_osd2_special_config(struct rts_video_osd2_attr *attr,
			struct rts_video_osd2_scfg *scfg, int blkidx);
